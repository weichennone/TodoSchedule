...
class TaskTree:
    ...
    def add_task(self, name, id=None):
        if name != "":
            sid = self._assign_id()
            task = Task(name, sid)
            if id:
    ...
    def finish_task(self, id, done=True):
        ...
        if parent_id == -1:
            ...
            # duration = (task.stop_end_time[-1] - task.start_time[-1]).total_seconds()
            # if len(self.task_stat):
            #     tag, elapse = self.task_stat[-1]
            #     if tag == 'work':
            #         self.task_stat[-1] = (tag, elapse + duration)
            #     else:
            #         self.task_stat.append(('work', duration))
            # else:
            #     self.task_stat.append(('work', duration))
            return task
        elif parent_id == -2:
    ...
    def construct_from_log(self):
        ...
        # re-allocate task id
        # keys = [k for k in self.task_dict.keys()] # old logger version v0.2
        # for k in keys: # old logger version v0.2
        for k in self.task_dict:
            self._idset.add(k)
            # old logger version v0.2
            # if isinstance(self.task_dict[k], list):
            #     task_name, subtasks = self.task_dict[k]
            #     self.task_dict[k] = Task(task_name, k)
            #     for k2 in subtasks:
            #         self.task_dict[k].add_successor(k2)
            #         self.task_dict[k2] = Task(subtasks[k2], k2)
            #         self.task_dict[k2].add_parent(k)
            #         self._idset.add(k2)
            # else:
            #     self.task_dict[k] = Task(self.task_dict[k], k)
